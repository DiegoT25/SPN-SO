...
int main()
{
    ...
    if(opcion==1){
        ...
        while(!feof(file)){
            processData p;
            sscanf(buffer,"%d,%d", &p.llegada, &p.burst, &p.prioridad);
            printf("Read: %d %d\n", p.llegada, p.burst, p.prioridad);
    ...
    for(time = 0; count != limit; time++)
    {
        ...
        for(i = 0; i < limit; i++)
        {
                ...
                else if(proceso[i].llegada <= time && proceso[i].burst == proceso[smallest].burst && proceso[i].burst > 0)
                {
                    if(proceso[i].prioridad < proceso[smallest].prioridad){
                        smallest = i;
                    }
                }
        }
        ...
        printf(" = %d\n", turn_times[j]);
        printf("Priority of process %d", j+1);
        printf(" = %d\n", proceso[j].prioridad);
